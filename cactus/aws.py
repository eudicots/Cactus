...
class AWSBucket(object):
	...
	def bucket(self):
		if not self._cache.has_key('bucket'):
			self._cache['bucket'] = self.connection.get_bucket(self.name)
		return self._cache.get('bucket', None)

	def create(self):
		logging.debug('Create bucket %s', self.name)
	...
	def configureWebsite(self):
		logging.debug('Configuring website endpoint %s', self.name)
		self.bucket().configure_website('index.html', 'error.html')
	
	def configureRedirect(self, url):
		logging.debug('Setup redirect %s -> %s', self.name, url)
...
class AWSDomain(object):
	...
	def createHostedZone(self):
		logging.debug('Creating hosted zone for %s', self.domain)
	...
	def _changeRecord(self, change, name, recordType, values, ttl):
		...
		logging.debug('%s record %s:%s in zone %s', change, name, recordType, self.domain)
	...
	def _changeAlias(self, change, name, recordType, aliasHostedZoneId, aliasDNSName, identifier, weight, comment):
		...
		logging.debug('%s alias %s:%s in zone %s', change, name, recordType, self.domain)
